                ...
                >
                  ...
                  <div v-else-if="item.eleName === 'workdir'">
                    <Workdir
                      :stage="data.stage"
                      :index="index"
                      :workdir="item.eleValues.workdir"
                      @setYamlCode="setYamlCode"
                    ></Workdir>
                  </div>
                  ...
                  <div v-else-if="item.eleName === 'deploy-contract'">
                    <DeployContract
                      :stage="data.stage"
                      :index="index"
                      :network="item.eleValues.network"
                      :private-key="(item.eleValues['private-key'])"
                      @setYamlCode="setYamlCode"
                    ></DeployContract>
                  </div>
                  <div v-else-if="item.eleName === 'deploy-ink-contract'">
                    <DeployInkContract
                      ...
                      :index="index"
                      :network="item.eleValues.network"
                      :private-key="(item.eleValues['mnemonic'])"
                      @setYamlCode="setYamlCode"
                    ></DeployInkContract>
                  </div>
...
const getTemplatesById = async (templateId: string) => {
  try {
    ...
    for (const key in config['stages']) {
      const obj = config['stages'][key]
      const steps: { eleName: string; eleValues: {} }[] = []
      if (obj['steps']) {
