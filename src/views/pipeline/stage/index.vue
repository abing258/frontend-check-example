<template>
  <div class="flex justify-between">
    <Breadcrumb :current-name="pathName" />
    <div>
      <a-button
        class="mr-2"
        @click="handleToEditPage()"
      >
        {{
      ...
      <a-button
        type="primary"
        @click="handleImmediateImplementation"
      >
        {{ $t("pipeline.stage.immediateImplementation") }}
      </a-button>
    </div>
  </div>
          ...
          <template v-else-if="column.key === 'action'">
            <div v-if="record.status == 1">
              ...
              <a
                ...
                @click.stop="handleStop(record.id)"
              >{{
