<template>
  <div class="bg-[#f9f9f9] mb-[20px] flex justify-between">
    <div class="flex flex-1">
      <img
        ...
        class="w-[24px] self-center mr-[4px] cursor-pointer"
        @click="backStep"
      />
      <span
        class="text-[18px] text-[#7B7D7B] self-center cursor-pointer hover:text-[#28c57c]"
        @click="backStep"
      >{{
        $t("component.back")
      }}</span>
      ...
      <img
        ...
        class="w-[20px] h-[20px] self-center ml-2 cursor-pointer"
        @click="visible = true"
      />
    </div>
    <a-button
      ...
      danger
      class="delete-btn"
      @click="deletePipelineList"
    >
      {{ $t("template.deletePipeline") }}
    </a-button>
  </div>
  <div class="bg-[#FFFFFF] rounded-[12px] leading-[24px]">
    ...
    <div
      class="mx-[24px] rounded-[12px] pb-[24px]"
      :style="{ height: mainHeight }"
    >
      <CodeEditor
        :read-only="false"
